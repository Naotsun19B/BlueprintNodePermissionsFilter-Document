...
これ有料プラグインなのでFab経由でのみ配布されます。  
審査完了後、Fab製品ページが有効化されます。  

1. [Fab](https://www.fab.com/ja/listings/74869306-565b-4051-a590-fd2d81ab2386)からインストールしてください。    
2. プラグインのインストール後に機能が使用できない場合は、プラグインが有効化されていない可能性がありますので、編集 > プラグイン からプラグインの有効のチェックが入っているかをご確認ください。  

## クイックスタート

インストールが完了したら、以下の手順に従ってプラグインの機能を確認できます。  

1. Unreal Editor で任意のプロジェクトを開きます。
2. `編集 > エディタ設定 > Blueprint Node Permissions Filter > Group Settings` に移動します。
3. グループツリービューで任意のグループを作成します。
4. 所属グループを、作成したグループに変更します。
5. `編集 > エディタ設定 > Blueprint Node Permissions Filter > [作成したグループ名]` に移動します。
6. ノードのフィルター状態を `Deny` に設定します。
7. 任意のブループリントグラフを開き、右クリックでアクションメニューを表示します。
8. アクセス拒否対象のノードがアクションメニューから非表示になっていることを確認します。
